...
class VolunteerController {
    ...
    func fetchVolunteerOpportunities(with searchTerm: String, completion: @escaping(Bool) -> Void) {
        ...
        let url = baseURL.withQueries(queries)?.appendingPathExtension("json")
...
//                    let volunteerDictionary = try jsonDecoder.decode(VolunteerJSONDictionary.self, from: data)
//                    let volunteerOpportunities = volunteerDictionary.opportunities.compactMap({$0})
